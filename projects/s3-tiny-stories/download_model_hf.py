#!/usr/bin/env python3
"""Download ESP32-S3 TinyStories model bundle from Hugging Face Hub.

Downloads model artifacts:
  - stories15M_q4.bin / model.bin (Quantized INT4 weights)
  - tokenizer.json (Tokenizer configuration and vocabulary)
  - metadata.json (Architecture metadata)
  - README.md (Model Card)

Optionally regenerates C headers in src/generated/ (vocab.h)
...
DEFAULT_REPO_NAME = "esp32-s3-tinystories"
CORE_FILES = ["stories15M_q4.bin", "model.bin", "tokenizer.json", "metadata.json", "README.md"]
...
def regenerate_c_headers():
    """Regenerate vocab.h from downloaded tokenizer.json."""
    gen_vocab = PROJECT_ROOT / "pc_tools" / "generate_vocab.py"

    if gen_vocab.exists():
        print("\nUpdating C decoding header (src/generated/vocab.h)...")
        subprocess.run([sys.executable, str(gen_vocab)], cwd=str(PROJECT_ROOT), check=True)


def main():
    parser = argparse.ArgumentParser(
        description="Download TinyStories model bundle from Hugging Face Hub."
    )
    parser.add_argument(
        "--repo-id",
        "-r",
        type=str,
        default=None,
        help="Hugging Face repository ID (<username>/<repo>). Defaults to authenticated user or nicholaswilde/esp32-s3-tinystories",
    )
    parser.add_argument(
        "--out-dir",
        "-o",
